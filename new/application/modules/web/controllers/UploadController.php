...
//文件保存目录路径
$save_path = APPLICATION_PATH.'/../public/upload/focus';
//文件保存目录URL
$save_url = '/upload/focus/';
//定义允许上传的文件扩展名
$ext_arr = array(
...
// 	}
	...
	$fileName = $dir.$curTimestamp.'_'.rand(10000,99999).'.'.$file_ext;
	...
	$file = realpath(APPLICATION_PATH.'/../public/upload'.$fileName);
	list($ret, $err) = $uploadMgr->putFile($token['token'], null, $file);
	if ($err !== null) {
		var_dump($err);
	} else {
		$urlImg = "http://img.caizhu.com/".$ret['hash'].'?imageView2/0/w/640';
		unlink($file);
	}
	$file_url	= $urlImg;
