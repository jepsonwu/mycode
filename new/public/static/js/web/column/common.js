...
function savaDraft(s){           // 需要有 definedPrompt000 支持
    // 保存草稿提示

    if(!s) return ;

    var savaFlag, closeFn;
...
function replayEmojiStr(s){
    ...
    for(var i=0,m,key,key2,exp,len=data.length; i<len; i++) {
        ...
        for (key in m) {
            ...
            s = s.replace(exp, '<img width=22 height=22 src="' + emojiObj.path + m[key] + '" />');
...
function originEmoji(text, width){
    ...
    var Emoji = {
        ...
        emojiPath: 'http://fe.caizhu.com/public/plugin/emoji01/',
        ...
        trans:  function(text) {
            ...
            text = text.replace(Emoji.reg, function(code) {
                return '<img width=' + width + ' class="emoji" style="vertical-align:middle" src="' + Emoji.emojiPath + Emoji._escapeToUtf32(code) + '.png">';
            });
...
function getEmojiStr(){
    var emojiStr = '<div class="cc-do"><textarea></textarea><div class="cc-emoji clearfix"><div class="cc-emojibox"><ul class="cc-emojidet hide clearfix">';
    ...
    for(var i=0; i<length; i++){
        ...
        path = emojiObj.path + a[b];
        emojiStr += '<li class="cc-select" data-str="'+b+'"><img src="'+ path +'"></li>';
    }

    emojiStr += '</ul></div><p>还可以输入<i class="shownum">500</i>字，按下Shift+Enter键发送</p></div><div class="cc-btn"><span class="cc-send">发送</span><span class="cc-cancel">收起</span></div></div>';
...
function parseUrl(key){
    ...
    for(var i=0,uLen=uArr.length;i<uLen;i++){
        uStr=uArr[i].split('=');
        if(uStr[0].toLowerCase() == key) return decodeURIComponent(String(uStr[1]));
    }
    return false;
}