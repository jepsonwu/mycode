...
//	正式环境API请求基础地址
	 ...
	 */
	public function __construct($merchantPrivateKey)
	{
...
//		$this->AES = new Crypt_AES(CRYPT_AES_MODE_ECB);
	...
	public function encrypt_data($data)
	{
		...
		$result = $this->RSA->decrypt(base64_decode($data));
		return $result;
	}
	/**
	 * 信用卡支付
	 */
	public function creditPayRequest($cardno, $validthru, $cvv2, $phone, $order_id, $transtime, $amount, $product_catalog,
	                                 $identity_id, $identity_type, $user_ip, $terminaltype, $terminalid, $callbackurl = '', $currency = 156, $product_name = '',
	                                 $product_desc = '', $other = '')
	{
		$query = array(
			'cardno' => $cardno,
		...
		return $this->post(YEEPAY_PAY_API, 'bankcard/credit/pay/request', $query);
	}

	/*
	* 储蓄卡支付请求  $cardpwd,'cardpwd'	=>	$cardpwd,
	
	*/
	public function debitPayRequest($cardno, $idcardtype, $idcard, $owner, $phone, $order_id, $transtime, $amount, $product_catalog,
	                                $identity_id, $identity_type, $user_ip, $terminaltype, $terminalid, $callbackurl = '', $currency = 156, $product_name = '',
	                                $product_desc = '', $other = '')
	{
		$query = array(
		...
		return $this->post(YEEPAY_PAY_API, 'bankcard/debit/pay/request', $query);
	}
	...
	public function bindPayRequest($bind_id, $order_id, $transtime, $amount, $product_catalog, $identity_id, $identity_type,
	                               $user_ip, $terminaltype, $terminalid, $callbackurl = '', $currency = 156, $product_name = '', $product_desc = '', $other = '')
	{
		$query = array(
	...
	public function post($type, $method, $query)
	{
	...
	protected function http($url, $method, $postfields = NULL)
	{
		...
		if (curl_errno($ci))
			throw new yeepayMPayException(curl_error($ci), curl_errno($ci));
