...
class Upload {
    ...
    public function upload($files='') {
        ...
        if(empty($files)){
            $this->error = $this->config['error_msg']["1001"];
            return false;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            if(in_array($ext, array('gif','jpg','jpeg','bmp','png','swf'))) {
                ...
                if(empty($imginfo) || ($ext == 'gif' && empty($imginfo['bits']))){
                    $this->error = $this->config['error_msg']["1002"];
                    continue;
                }
            }
    ...
    private function check($file) {
        ...
        /* 无效上传 */
        if (empty($file['name'])){
            $this->error = $this->config['error_msg']["1003"];
        }

        /* 检查是否合法上传 */
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->error = $this->config['error_msg']['1004'];
            return false;
        }

        /* 检查文件大小 */
        if (!$this->checkSize($file['size'])) {
            $this->error = $this->config['error_msg']["1005"];
            return false;
        }
        ...
        if (!$this->checkMime($file['type'])) {
            $this->error = $this->config['error_msg']["1006"];
            return false;
        }

        /* 检查文件后缀 */
        if (!$this->checkExt($file['ext'])) {
            $this->error = $this->config['error_msg']["1007"];
            return false;
        }
