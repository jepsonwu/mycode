$(function() {
	...
	$("#tabs li a").click(function(){
		...
		if ( tabNow==0 ) {
			$("#qxdty").css("display", 'inline-block');
		}
		else {
			$("#qxdty").css("display", 'none');
		}
	});
	// 默认插页点击
	...
	$("#sele_role").change(function(){
		window.location.href = CONTROLLER + "/accredit/id/" + $(this).val();
	});
	
	// 应用选择
	$("#sele_yy").change(function(){
		if ( $(this).val() != '' ) {
			$.post( CONTROLLER+"/getNodesByPid/", {
				pid: $(this).val(),
	...
	$("#btn_save").click(function(){
		...
		//
		var url = CONTROLLER + "/saveAccess/";
		var param = {'role_id':role_id,'node_ids':node_ids,'level':level,'pid':pid};
		my_ajax_sub(url, param);
	});
	
	// 权限等同与（权限拷贝）
	$("#btn_the_same_as").click( function(){
		var to = $("#sele_role").val();
		var from = $("#from_role").val();
		//
		if ( to == from ) {
			alert( '目的权限组和来源权限组不能相同！' );
			return false;
		}
		//
		my_ajax_post( CONTROLLER + "/theSameAs", { 'to':to, 'from':from } );
	});
