...
class CommentController extends CommonController {
	...
	public function list_get_html() {
		...
		$comment_info = $comment_model->join('left join ft_users u on s.sid=u.id')
			->field('s.id,u.avatar,u.name,u.id user_id,s.create_time,s.content')->page($this->page,$this->listrows)
	...
	public function teacher_get_html() {
		...
		$comment_info = $comment_model->alias('t')->join('left join ft_users u on t.sid=u.id')
		...
		$result['list'] = $comment_info;
		// 记录总数
		$count = $comment_model->alias('t')->join('left join ft_users u on t.sid=u.id')
