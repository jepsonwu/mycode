...
class OrdersController extends CommonController
{
	...
	// 资金流常量
	const FUND_FLOW_MODE_MINUS = 2;
	const FUND_FLOW_TYPE_CALL = 2;
...
//						$value = array_diff($this->_default['orders_fields'], $value);
	...
	protected $read_get_html_conf = array(
		...
		"check_fields" => array(
			array("orders:fields",
				"order_id,total_amount,paid_amount,create_time,paid_time,status,sid,tid,pay_type,recording_url",
				"!paid_amount,paid_time"),
			array("calls:fields", "called_time,begin_time"),
			array("users:fields", "name,gender,avatar,nationality,type"),
			array("teacher_comments:fields", "level,content"),
	...
	public function settlement_post_html()
	{
		...
		//通话时长
		$called_time = $this->called_time >= 60 ? $this->called_time - $this->called_time % 60 : 0;
		...
		if ($total_amount > 0) {
			...
			// 计算通话费用并扣除余额
			$balance = M('Users')->getFieldById($user_id, 'balance');
			$call_amount = $balance > $total_amount ? $total_amount : $balance;
			$balance = $balance - $total_amount > 0 ? $balance - $total_amount : 0;
			$user_result = M('Users')->where("id={$user_id}")->save(array('balance' => $balance));
			if ($user_result !== false) {
				// 获取通话ID
				$call_id = M('Calls')->getFieldByOrderId($this->order_id, 'id');
				// 新增用户资金流
				$fund_data = array(
					'user_id' => $user_id,
					'relation_id' => $call_id,
					'mode' => self::FUND_FLOW_MODE_MINUS,
					'type' => self::FUND_FLOW_TYPE_CALL,
					'amount' => $call_amount,
					'create_time' => time()
				);
				$fund_result = M('UserFundFlow')->add($fund_data);
				if ($fund_result === false) $model->rollback();
			} else {
		...
		// 通话费用
		$call_amount = $call_amount ?: 0;
		//结算订单
		$result = $model->where("order_id='" . $this->order_id . "'")->
		save(array(
			"total_amount" => $total_amount,
			"paid_amount" => $call_amount,
