...
class TeachersController extends CommonController
{
	 ...
	 * 老师列表更新时长
	 */
	public function updateTime_get_html()
	{
		$model = M('Orders');
		foreach (array("online", "busy", "offline") as $status) {
			echo $status;
			...
			$where = array();
			$teachers_tmp = S("teacher_{$status}_list");
			print_r($teachers_tmp);
			foreach ($teachers_tmp as $tid => $called_time) {
				$tids[] = $tid;
			}
			$where['status'] = array('neq',1);
			$where['tid'] = array('in',$tids);
			$teachers_tmp_new = $model->where($where)->group('tid')->field('tid,sum(called_time) as time')->select();
			foreach ($teachers_tmp_new as $val) {
				$teachers_tmp[$val['tid']] = $val['time'];
			}
			S("teacher_{$status}_list",$teachers_tmp);
		}
	}
}