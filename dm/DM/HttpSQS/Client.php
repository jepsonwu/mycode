...
class DM_HttpSQS_Client
{
	...
	{
		$config = DM_Controller_Front::getInstance()->getConfig()->httpsqs;
		$this->httpsqs_host = $config->host;
		$this->httpsqs_port = $config->port;
		$this->httpsqs_auth = $config->pass;
		$this->httpsqs_charset = 'utf-8';
	}
	...
	public function put($queue_name, $queue_data)
	{
		...
		$result = $this->http_post("/?auth=" . $this->httpsqs_auth . "&charset=" . $this->httpsqs_charset . "&name=" . urlencode($queue_name) . "&opt=put", $queue_data);
	...
	public function get($queue_name)
	{
		...
		return $result["data"] == 'HTTPSQS_GET_END' ? false : $result['data'];
	}
	...
	public function gets($queue_name)
	{
		...
		return $result["data"] == 'HTTPSQS_GET_END' ? false : $result;
	}
