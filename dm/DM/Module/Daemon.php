 ...
 * @since 2014/10/24
 */
abstract class DM_Module_Daemon extends DM_Module_Base
{
    //任务执行间歇性休息时间
    //子类可以通过继承覆盖
     ...
     * 单进程防并发守护
     * 
     * @var DM_Module_Daemon_SingleProcessGuard
     */
     ...
     *
     * @var DM_Module_Daemon_ReleaseCheck
     */
     ...
     * 内存限额检测机制
     *
     * @var DM_Module_Daemon_MemoryCheck
     */
    ...
    public final function daemonRun()
    {
        try {
            ...
            self::getLog()->add('Find DM_Module_Daemon::daemonRun Exception:'.$e->__toString());
            //退出前刷新日志
            DM_Module_Log::forceFlush();
            //print_r($e);
            die();
        }
    }
    ...
    {
        $front=DM_Controller_Front::getInstance();
        if ($front->getDb()){
            $front->getDb()->query('select 1+1;');
        }

        if ($front->hasSlaveDB()){
            $front->getHashSlaveDB()->query('select 1+1;');
        }
    }
    ...
    {
        return DM_Module_Log::create($this->service);
    }
    
    protected function getSingleGuard()
    {
        if ($this->singleGuard===NULL){
            $this->singleGuard=new DM_Module_Daemon_SingleProcessGuard($this->service);
        }
    ...
    protected function getReleaseCheck()
    {
        if ($this->releaseCheck===NULL){
            $this->releaseCheck=new DM_Module_Daemon_ReleaseCheck($this->service);
        }
    ...
    protected function getMemoryCheck()
    {
        if ($this->memoryCheck===NULL){
            $this->memoryCheck=new DM_Module_Daemon_MemoryCheck($this->service);
        }
