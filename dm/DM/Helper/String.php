...
class DM_Helper_String
{
	...
	public static function escapeVar(&$var)
	{
		if (!empty($var)) {
			if (is_array($var)) {
				array_walk_recursive($var, array('DM_Helper_String', 'escapeVar'));
	 ...
	 * @param string $string
	 * @param int $len
	 * @param bool $dot 是否加小数点
	 */
	public static function getByLen($string, $len, $dot = 1)
	{
		if (strlen($string) <= $len) return $string;

		return mb_substr($string, 0, $len, 'utf-8') . ($dot ? '...' : '');
	}
