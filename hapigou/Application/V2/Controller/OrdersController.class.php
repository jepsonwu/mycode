...
class OrdersController extends CommonController
{
	...
	public function list_get_html()
	{
		...
		//状态 这里要注意  status为0时候
		$this->status !== "" && $where['status'] = $this->status;

		//处理一个事件 orders 表 fields 为空 或者不包含关联信息 todo
//		$value_diff=false;
//		if(!isset($this->_default['orders_fields']))
//			$this->_default['orders_fields']=array();

//		if()
//		$value_diff=true;
//			$this->_default['orders_fields']=array("order_id","sid","tid");
//		}
		...
		if ($count) {
			...
			//查找结果集  有两种情况  当有status条件的时候
			//当为学生，排序规则：1.未付款 其他按时间
			//当位老师，排序规则：1.未结算 其他按时间
			if ($this->status !== "") {
				$result = $model->where($where)->page($this->page, $this->listrows)
					->field($this->_default['orders_fields'])->order("create_time desc")->select();
			} else {
				//不同角色 对应条件不一样
				$add_where = ($this->role == 1) ? self::ORDER_STATUS_NEW : self::ORDER_STATUS_PAY;

				//查询第一层所有的总数 第一层limit可取总数
				$where['status'] = $add_where;
				$count_one = $model->where($where)->count(1);
				$count_limit = 0;

				if ($count_one > 0) {
					//没有数据返回NULL
					$result = $model->where($where)->field($this->_default['orders_fields'])
					...
					$count_limit = count($result);
				}

				//第二层
				if ($count_limit < $this->listrows) {
					//第二层起始位置 todo 检查
					$offect_two = intval(($this->page - 1) * $this->listrows) + $count_limit - $count_one;

					$where['status'] = array("neq", $add_where);
					$result_two = $model->where($where)->field($this->_default['orders_fields'])
						->limit($offect_two, $this->listrows - $count_limit)
						->order("create_time DESC")->select();

					$result_two && $result = array_merge($result, $result_two);
				}
			}
